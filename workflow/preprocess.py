...
def extract_from_beta_with_freq(raw, bench_ref, rolling_freq: int, ewm_span: int):
    """
    :param raw: pl.DataFrame
    :param bench_ref: pl.DataFrame
    :param rolling_freq: int represents minute
    :param ewm_span: int  window
    """
    ...
    data_joined = raw.join(
        ...
        log_ret_raw = pl.col("close").log().diff().fill_null(0.0), # avoid missing data cause 0.0
    ...
    beta_df = data_joined.group_by_dynamic( # agg drop original column and keep new
        ...
        xy_ewm = (pl.col("ret_freq") * pl.col("ret_bench_freq")).ewm_mean(span=span, ignore_nulls=True), # avoid jump
        ...
        raw_beta = pl.when(pl.col("var_y") > 1e-8) # eps avoid np.nan
        ...
        beta = pl.min_horizontal(
                   pl.max_horizontal(
                    pl.col("raw_beta"), pl.col("beta_lower").fill_null(-3.0)
                    ), 
