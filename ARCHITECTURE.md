# Backtest 架构与模块交互链路文档

> 本文档基于 `backtest/` 源码梳理，涵盖核心模块职责、类继承关系、运行时调用链路及数据流向。

---

## 1. 总体架构概览

`backtest` 是一个量化回测与仿真交易框架，设计上参考了 **backtrader** 的 Lines/元类架构，同时深度集成了 `bt_sdk` 的协议层（`OrderBody`、`SnapshotBody` 等），以支持高性能回测与在线交易的统一抽象。
