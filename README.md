...
pipeline which has probably positive return can be finetuned to obtain better performance

strategy protype must be selected by human criteria --- bayies prior

when to sell ---> opposite signal is triggered (one or all) 

before_trading_start ---> a. position need to be adjusted (close or hold or increase)

spider:
    1. adjustement (sina)
    2. delist/suspend (exchange sh/sz)
